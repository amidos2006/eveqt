...
import eveqtree.operators.binary.BinaryOperator;
import eveqtree.operators.unary.UnaryOperator;
import eveqtree.terminals.*;
...
public class EvEqTree {
     ...
     */
    public static String generateRandomStringEquation(EquationParser parser, int maxDepth, double constProb, double unaryProb) {
	...
	if(rnd.nextDouble() < unaryProb){
	    return parser.getRandomOperator(true) + "(" + generateRandomStringEquation(parser, maxDepth - rnd.nextInt(maxDepth) - 1, constProb, unaryProb) + ")";
	}
	
	return parser.getRandomOperator(false) + "(" + generateRandomStringEquation(parser, maxDepth - rnd.nextInt(maxDepth) - 1, constProb, unaryProb) + "," + 
		generateRandomStringEquation(parser, maxDepth - rnd.nextInt(maxDepth) - 1, constProb, unaryProb) + ")";
    }
     ...
     */
    public static EquationNode generateRandomTreeEquation(EquationParser parser, int maxDepth, double constProb, double unaryProb) throws Exception {
	return parser.parse(generateRandomStringEquation(parser, maxDepth, constProb, unaryProb));
    }
	...
	return generateRandomTreeEquation(parser, maxDepth, 0.5, 0.5);
    }
	...
	int targetDepth = parser.getRandom().nextInt(clone.getTreeDepth()-1);
     ...
     */
    public static EquationNode insertNode(EquationParser parser, EquationNode eq, int maxDepth, double constProb, double unaryProb) throws Exception {
	EquationNode clone = (EquationNode) eq.clone();
	...
	if(rnd.nextDouble() < unaryProb) {
	    ...
	    newChild = parser.parse(parser.getRandomOperator(false) + "(" + 
		    selected + "," + generateRandomStringEquation(parser, maxDepth-selected.getCurrentDepth(), constProb, unaryProb) + ")");
	}
	if(parent == null) {
	    return newChild;
	}
	...
	return insertNode(parser, eq, maxDepth, 0.5, 0.5);
    }
